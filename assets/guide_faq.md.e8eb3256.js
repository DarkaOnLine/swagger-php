...
</code></pre></div><p>The <code>-b</code> allows to execute some extra PHP code to load whatever is needed to register your apps classloader with PHP.</p><div class="warning custom-block"><p class="custom-block-title">WARNING</p><p>One common case for this type of error is when trying out annotations in a standalone single file. Typically, this means it will not use namespaces or confirm to any other autoloading standards.</p><p>In this case the <code>-b</code> option can also be used to autoload the actual file.</p><p>Please note that you still need to provide the file (or its folder) as the target.</p><div class="language-shell"><pre><code>openapi <span class="token parameter variable">-b</span> src/test.php src/test.php
</code></pre></div></div><h3 id="namespace-mismatch" tabindex="-1">Namespace mismatch <a class="header-anchor" href="#namespace-mismatch" aria-hidden="true">#</a></h3><p>Another reason for this error could be that your class actually has the wrong namespace (or no namespace at all!).</p><p>Depending on your framework this might still work in the context of your app, but the composer autoloader alone might not be able to load your class (assuming you are using composer).</p><h2 id="no-output-from-openapi-command-line-tool" tabindex="-1">No output from <code>openapi</code> command line tool <a class="header-anchor" href="#no-output-from-openapi-command-line-tool" aria-hidden="true">#</a></h2><p>Depending on your PHP configuration, running the <code>openapi</code> command line tool might result in no output at all.</p><p>The reason for this is that <code>openapi</code> currently uses the <a href="https://www.php.net/manual/en/function.error-log.php" target="_blank" rel="noopener noreferrer"><code>error_log</code></a> function for all output.</p><p>So if this is configured to write to a file, then it will seem like the command is broken.</p>`,35)]))}var m=e(o,[["render",i]]);export{h as __pageData,m as default};
